...
awk -F '-' '{a[$1]++} END {for(b in a) print b}' nama2brev.txt >> hasil.txt
```
Untuk nomer 2b, pengerjaan dimulai dengan pengolahan data dari Laporan-TokoShiShop.tsv yang bertujuan untuk memilah data dengan mendapatkan transaksi yang terjadi di tahun 2017 dengan cara membuat output file memiliki separator "-" kemudian mengambil kolom yang menunjukkan angka 17 atau berarti tahun 2017, kemudian hasilnya dioutputkan dalam sebuah file yang kemudian diolah untuk diambil data yang terjadi di Albuquerque setelah itu hasil pengolahan tersebut dijadikan output ke hasil.txt
...
```
Nomer 2c dikerjakan dengan memilih record dari setiap segment kemudian dijadikan output pada sebuah file, file tersebut akhirnya mengandung sejumlah data dari tiap segment yang kemudian dihitung jumlahnya untuk mengindikasikan berapa banyak transaksi yang terjadi pada masing masing sement. Hasil tersebut kemudian dibandingkan atau dicari hasilnya yang paling kecil untuk dijadikan output pada hasil.txt
